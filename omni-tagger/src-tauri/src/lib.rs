    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            *app.state::<AppState>().config.lock().unwrap() = config.clone();
            ...
            init_model_loader(app.handle(), config);
...
fn init_model_loader(app_handle: &tauri::AppHandle, config: AppConfig) {
    ...
    let app_handle_gui = app_handle.clone();
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if let Err(e) =
            model_manager::check_and_download_models(&app_handle_gui, &model_path, &tags_path).await
        {
