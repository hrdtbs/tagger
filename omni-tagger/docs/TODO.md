...
- [x] **AI Inference Engine**: Replace mock implementation in `tagger.rs` with real `ort` (ONNX Runtime) integration.
    ...
    - [x] Implement image preprocessing (Resize to 448x448, BGR Conversion retaining [0, 255] range).
...
## Known Issues & Bugs
- [ ] **Arbitrary File Deletion Vulnerability**: The `--delete-after` CLI flag accepts any file path without validation, allowing arbitrary file deletion if exploited by malicious arguments. This must be restricted to operate only on temporary files with a specific prefix.
...
- [ ] **Security (Arbitrary File Deletion Prevention)**: Restrict the `--delete-after` CLI flag to only operate on temporary files generated by the application (e.g., checking for specific prefixes and ensuring paths are within the OS temporary directory).
- [ ] **Security (SSRF Prevention)**: Implement strict URL validation for `--process-url` and Native Messaging requests. Only allow `http` and `https` schemes, disable HTTP redirects, and reject localhost/private network IPs.
