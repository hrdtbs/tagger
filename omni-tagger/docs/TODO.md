...
- [ ] **Security (HTTP OOM Prevention)**: Implement a maximum download size limit (`content-length` check and stream reading limits) in `process_image_url` to prevent memory exhaustion from giant or infinite HTTP responses.
- [ ] **Security (Arbitrary File Deletion)**: Restrict the `--delete-after` argument so that it can only delete files within the OS's specific temporary directory (e.g. `env::temp_dir()`) to prevent arbitrary file deletion vulnerabilities.
