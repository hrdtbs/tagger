...
- [x] **Browser Extension Support (Native Messaging)**:
    ...
    - [ ] **Unregister Native Host (Backend Logic)**: Implement logic to remove Native Messaging Host registry keys/manifests (currently only registration is supported).
...
- [ ] **Manual Verification (Windows)**:
    ...
    - [x] Test "Register Host" adds the manifest file and registry key. (Verified via unit tests in `registry.rs` for Chrome, Edge, Brave, and Firefox manifest generation)
